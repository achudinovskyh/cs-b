...
{
    if(hasNext)node = findMinAfterThis(this->node->key,root);
...
typename MyMap<K,V>::Node *MyMap<K,V>::iterator::findMinAfterThis(K incValue, Node *ptr)
{
    if(ptr->key > incValue){
        if(ptr->left != NULL && findMaxKey(ptr->left) > incValue){
            findMinAfterThis(incValue,ptr->left);
        }else{
            return ptr;
        }
    }else{
        ...
        findMinAfterThis(incValue,ptr->right);
    }
}
